"""
OPE Figure 2 plotting via src.visualization (read stats CSV → plot → save PDF).
"""
...
from src.visualization import plot_ope_figure2_from_stats
...
def ope_figure2_path(exp_cfg, suffix=None):
    base = f"OPE_Figure2_{exp_cfg.SETTING}"
    if suffix is not None:
...
def regenerate_ope_figure2(stats_file, save_path):
    """Regenerate OPE Figure 2 from statistics CSV (same workflow as regenerate_figures.py)."""
    print("=" * 60)
    print("Regenerate OPE Figure 2: MSE vs T")
    print("=" * 60)
    ...
    plot_ope_figure2_from_stats(_stats_for_visualization(df_stats), save_path=save_path)

    print(f"\nOPE Figure 2 saved to: {save_path}")
    return True
