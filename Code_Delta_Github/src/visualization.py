...
def plot_policy_heatmap(policy_ipw, policy_ai, policy_dro, setting_name="Experiment"):
    ...
    filename = f"heatmap_{setting_name}.png"
...
def plot_dropl_vs_n(df, metric_col='Hat Q_DRO', title_suffix='', save_path=None):
    """
    Plot similar to Kallus22 Figure 3: Distributionally Robust Value vs N
    ...
    save_path : str, optional
        Save path
    """
    print(f"\n[Visualization] Generating DROPL vs N plot (Kallus22 Figure 3 style)...")
    ...
    if has_delta_test:
        ...
        for model in all_models:
            if 'DRO(δ=0.2)' in model or 'DRO(δ=0.2' in model:
        ...
        for idx, delta_test in enumerate(all_deltas):
            ...
            for model in preferred_order:
                ...
                if 'Ai2024' in model:
                    ...
                    label = 'DRO-IPW'
                elif 'DRO(δ=0.2' in model or 'DRO(δ=0.2)' in model:
                    label = 'CD$R^2O^2$PL-CP'
                else:
                    label = model
            ...
            if idx == 0:  # Only show y-axis label on first subplot
                ax.set_ylabel('Distributionally Robust Value $R_\\delta$', fontsize=12)
            ax.set_title(f'$\\delta = {delta_test}$', fontsize=14, fontweight='bold')
        ...
        ax.set_xlabel('T', fontsize=12)
        ax.set_ylabel('Distributionally Robust Value $R_\\delta$', fontsize=12)
        ax.set_title(f'Distributionally Robust Policy Learning vs N{title_suffix}', fontsize=14, fontweight='bold')
        ax.legend(loc='best', fontsize=10)
        ax.grid(True, alpha=0.3)
    ...
    if save_path:
        ...
        filename = f"dropl_vs_n_{metric_col.lower()}{title_suffix}.png"
...
def plot_dropl_vs_delta(df, metric_col='Hat Q_DRO', title_suffix='', save_path=None):
    ...
    ax.set_xlabel('Test Delta $\\delta$', fontsize=12)
    ax.set_ylabel('Distributionally Robust Value $R_\\delta$', fontsize=12)
    ax.set_title(f'Distributionally Robust Policy Learning vs Test Delta{title_suffix}', fontsize=14)
    ax.legend(loc='best', fontsize=10)
    ax.grid(True, alpha=0.3)
    ...
    if save_path:
        ...
        filename = f"dropl_vs_delta_{metric_col.lower()}{title_suffix}.png"
        plt.savefig(filename, dpi=300, bbox_inches='tight')
        print(f"Saved figure to {filename}")
...
def plot_behavior_vs_target_policy(behavior_policy_func, target_policies_dict, setting_name="BehaviorVsTarget"):
    ...
    filename = f"BehaviorVsTarget_{setting_name}.png"
    plt.savefig(filename, dpi=300, bbox_inches='tight')
    print(f"Saved figure to {filename}")


def plot_ope_figure2(df, save_path=None):
    """
    Plot similar to Kallus22 Figure 2: MSE vs T (training sample size)
    ...
    save_path : str, optional
        Save path (default: 'OPE_Figure2.png')
    """
    print(f"\n[Visualization] Generating OPE Figure 2 (MSE vs T)...")
    ...
    # Define colors and linestyles (directly specified, referencing Figure3 color scheme)
    method_colors = {}
    method_linestyles = {}
    method_labels = {}
