...
def rule_of_thumb_bandwidth(P, exponent_denom=5.0):
    """h = 1.06 * std(P) * n^{-1/exponent_denom}."""
    ...
    if std_p < 1e-6:
        std_p = 1.0
    return float(1.06 * std_p * (n ** (-1.0 / float(exponent_denom))))


def silverman_bandwidth(P):
    """Silverman's rule: h = 1.06 * std(P) * n^{-1/5}."""
    return rule_of_thumb_bandwidth(P, exponent_denom=5.0)


def resolve_bandwidth(P, mode="silverman", min_bandwidth=None):
    """
    Resolve kernel bandwidth h for OPL/OPE.

    Parameters
    ----------
    mode : str or float
        ``'silverman'`` for h = 1.06 * std(P) * n^{-1/5}, or a numeric exponent
        denominator d for h = 1.06 * std(P) * n^{-1/d} (e.g. 4.9, 2.9).
    min_bandwidth : float, optional
        Lower bound on h (defaults to ``config.MIN_BANDWIDTH``).
    """
    if min_bandwidth is None:
        min_bandwidth = config.MIN_BANDWIDTH
    if isinstance(mode, str) and mode.lower() == "silverman":
        h = silverman_bandwidth(P)
    elif isinstance(mode, (int, float)):
        h = rule_of_thumb_bandwidth(P, exponent_denom=float(mode))
    else:
        raise ValueError(f"Unknown bandwidth mode: {mode!r}")
    return float(max(min_bandwidth, h))
