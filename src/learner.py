...
class BaseLearner:
    def __init__(self, policy_net, bandwidth, delta=None):
        self.policy = policy_net
        self.h = bandwidth
        # Robustness radius for DRO learners; None for non-robust baselines (e.g. Ai206).
        self.delta = delta
...
class Ai2026Learner(BaseLearner):
    """
    Non-robust OPL baseline (Ai et al., 2026 double-debiased estimator).

    Training does not use δ. For fair comparison with DRO methods, synthetic OPL
    evaluation reports Hat Q_DRO via BaseLearner.get_optimal_alpha(..., delta_test)
    with δ passed at eval time only (see experiments/opl/opl_common.evaluate_metrics).
    """

    def __init__(self, policy_net, outcome_model, bandwidth):
        super().__init__(policy_net, bandwidth)
    ...
    def train_step(self, X_batch, P_batch, Y_batch, Propensity_batch):
        """
        Double-Debiased (DD) estimator from Ai et al. (2026)
